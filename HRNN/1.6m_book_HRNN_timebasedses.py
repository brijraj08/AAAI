...
# %%
#sessions with 30 days(long-period dataset)
...
"""
def process_sequences_v2(df):
        # Function to process each user's sequences
    def process_user(user_df):
        #user_df.sort_values(by='session_id', inplace=True)
        while True:
                        # Handle case when there's only one or no sequence left
            if len(user_df) <= 1:
                break
            previous_sequences = user_df.iloc[:-1]['sequence'].explode().unique()  # All items in previous sequences
            last_sequence_items = user_df.iloc[-1]['sequence']
                                    # Filter last sequence items based on presence in previous sequences
            filtered_last_sequence = [item for item in last_sequence_items if item in previous_sequences]

            if len(filtered_last_sequence) > 1:
                user_df.at[user_df.index[-1], 'sequence'] = filtered_last_sequence
                break
            elif len(filtered_last_sequence) == 1:
                if len(user_df) > 1:
                    user_df.iloc[-2]['sequence'].extend(filtered_last_sequence)
                user_df = user_df.iloc[:-1]
            else:
                user_df = user_df.iloc[:-1]
            if user_df.empty or len(user_df.iloc[-1]['sequence']) == 0:
                break
        return user_df
    processed_groups = [process_user(group) for _, group in df.groupby('user_id')]
    return pd.concat(processed_groups)
processed_dataset = process_sequences_v2(dataset)
