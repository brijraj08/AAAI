...
#    metrics = np.zeros(len(evaluation_functions))
    metrics_5 = np.zeros(len(evaluation_functions))
    metrics_10 = np.zeros(len(evaluation_functions))
    metrics_20 = np.zeros(len(evaluation_functions))
    ...
    with tqdm(total=len(test_sequences)) as pbar:
        for i, test_seq in enumerate(test_sequences):
            ...
            if scroll:
                ...
                #                                           top_n,
                #                                           step)
               t= sequence_sequential_evaluation(recommender,
                                                          test_seq,
                                                          evaluation_functions,
                                                          user,
                                                          given_k,
               ...
               metrics_20 += t[2]
                ...
                #                              look_ahead,
                #                              top_n)
                t += evaluate_sequence(recommender,
                                             test_seq,
                                             evaluation_functions,
                                             user,
                                             given_k,
            ...
            pbar.update(1)
    return [metrics_5 / len(test_sequences), metrics_10 / len(test_sequences), metrics_20 / len(test_sequences)]
...
def evaluate_sequence(recommender, seq, evaluation_functions, user, given_k, look_ahead, top_n):
    ...
    r_5 = recommender.recommend(user_profile, user)[:5]
    r_10 = recommender.recommend(user_profile, user)[:10]
    r_20 = recommender.recommend(user_profile, user)[:20]
    if not r_5:
        # no recommendation found
        return np.zeros(len(evaluation_functions))
    #reco_list = recommender.get_recommendation_list(r)
    reco_list_5 = recommender.get_recommendation_list(r_5)    
    reco_list_10 = recommender.get_recommendation_list(r_10)
    reco_list_20 = recommender.get_recommendation_list(r_20)
    #tmp_results = []
    tmp_results_5 = []
    tmp_results_10 = []
    tmp_results_20 = []
    for f in evaluation_functions:
        #tmp_results_5.append(f(ground_truth, reco_list))
        tmp_results_5.append(f(ground_truth, reco_list_5))
        tmp_results_10.append(f(ground_truth, reco_list_10))
        tmp_results_20.append(f(ground_truth, reco_list_20))
    #return np.array(tmp_results_5, tmp_results_10, tmp_results_20)
    return [tmp_results_5, tmp_results_10, tmp_results_20]
...
#    eval_res = 0.0
    ...
    for gk in range(given_k, len(seq), step):
        print("Return here=", evaluate_sequence(recommender, seq, evaluation_functions, user, gk, look_ahead, top_n))
        eval_res_5 += evaluate_sequence(recommender, seq, evaluation_functions, user, gk, look_ahead, top_n)[0]
        eval_res_10 += evaluate_sequence(recommender, seq, evaluation_functions, user, gk, look_ahead, top_n)[1]
        eval_res_20 += evaluate_sequence(recommender, seq, evaluation_functions, user, gk, look_ahead, top_n)[2]
        eval_cnt += 1
    return [eval_res_5 / eval_cnt, eval_res_10 / eval_cnt,eval_res_20 / eval_cnt]
