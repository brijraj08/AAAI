...
def last_session_out_split(data,
    ...
    #for i in range(0, len(data)):
        #train['sequence'][i]=train['sequence'][i][:-1]
        #test['sequence'][i]=test['sequence'][i][-2:]

    sessions = data.sort_values(by=[user_key, time_key]).groupby(user_key)[session_key]
    #sessions = data.sort_values(by=[user_key, session_key]).groupby(user_key)[session_key]
...
def last_session_out_split(data,
                        user_key='user_id',
                       session_key='session_id', time_key='ts'):
         
         #Assign the last session of every user to the test set and the remaining ones to the training set
        
         # data = data.sort_values(by=[user_key, time_key]).groupby(user_key)[session_key]
         train= data.copy()
         test=data.copy()
         for i in range(0, len(data)):
             train['sequence'][i]=train['sequence'][i][:-1]
