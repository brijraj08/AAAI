...
def data_expansion(sequences, history_length):
    ...
    return csc_matrix((np.ones(len(row_indeces), dtype=np.int8), (row_indeces, col_indeces)),
                      shape=(row, (history_length + 2) * len(items_mapping))), items_mapping
...
def user_profile_expansion(user_profile, history_length, items_mapping):
    ...
    return csc_matrix((np.ones(len(row_indeces)), (row_indeces, col_indeces)),
                      shape=(1, (history_length + 1) * len(items_mapping)))
