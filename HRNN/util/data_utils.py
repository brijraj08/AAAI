...
def create_seq_db_filter_top_k1(path, topk=0, last_months=0):
...
#    aggregated = groups['item_id'].agg('sequence': [lambda x: list(map(str, x))])
    aggregated  = groups.item_id.agg({lambda x: list(map(str, x))}).rename(columns={'<lambda>':"sequence"})
    #aggregated = groups.item_id.agg([lambda x: list(map(str, x))])    
...
def load_and_adapt(path, last_months=0):
    ...
    print("Column names here are=", data.columns)
#    col_names = ['session_id', 'user_id', 'item_id', 'ts'] + data.columns.values.tolist()[4:]
    col_names = ['user_id', 'item_id', 'rating', 'ts','session_id'] #+ data.columns.values.tolist()[4:]
