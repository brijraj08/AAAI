...
FILENAME_PATTERN ='1.6m_book_homodataset_tr4rec2001_splitorgts1.csv'
...
pandas_df = pd.read_csv(DATA_PATH, sep=',',
                                names=['item_id', 'session_id','timestamp'],
                                dtype={'item_id':'int','session_id':'int','timestamp':'int64'}, header=0)
...
workflow.fit_transform(dataset).to_parquet(os.path.join(DATA_FOLDER, "bookhomoses2001_processed_nvt"))

workflow.output_schema

workflow.save(os.path.join(DATA_FOLDER, "bookhomoses2001_workflow_etl"))

sessions_gdf = cudf.read_parquet(os.path.join(DATA_FOLDER, "bookhomoses2001_processed_nvt/part_0.parquet"))
...
save_time_based_splits(data=nvt.Dataset(sessions_gdf),
                       output_dir=os.path.join(DATA_FOLDER, "bookhomoses2001_preproc_sessions_by_year"),
