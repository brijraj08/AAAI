...
DATA_FOLDER=""
FILENAME_PATTERN ='lastfm_tssess900.csv'
#FILENAME_PATTERN ='1ml_cluster_sessions.csv'
...
selected_features = sess_id + groupby_features['item_id-count'] + groupby_features_truncated + month_index
...
workflow.fit_transform(dataset).to_parquet(os.path.join(DATA_FOLDER, "lastfmtimebased2_processed_nvt"))

workflow.output_schema

workflow.save(os.path.join(DATA_FOLDER, "lastfmtimebased2_workflow_etl"))

sessions_gdf = cudf.read_parquet(os.path.join(DATA_FOLDER, "lastfmtimebased2_processed_nvt/part_0.parquet"))

sessions_gdf = sessions_gdf[sessions_gdf.month_index>=0]

#print(sessions_gdf.head(3))

from transformers4rec.utils.data_utils import save_time_based_splits
save_time_based_splits(data=nvt.Dataset(sessions_gdf),
                       output_dir=os.path.join(DATA_FOLDER, "lastfmtimebased2_preproc_sessions_by_month"),
                       partition_col='month_index',
