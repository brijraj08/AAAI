...
DATA_FOLDER=""
FILENAME_PATTERN ='10_feb_20_catg_cd.csv'
...
pandas_df = pd.read_csv(DATA_PATH, usecols=['SessionId', 'ItemId', 'Time'])
pandas_df.columns = ['session_id', 'item_id', 'timestamp']  # Rename columns after loading
...
workflow.fit_transform(dataset).to_parquet(os.path.join(DATA_FOLDER, "amazonhomo_processed_nvt"))

workflow.output_schema

workflow.save(os.path.join(DATA_FOLDER, "amazonhomo_workflow_etl"))

sessions_gdf = cudf.read_parquet(os.path.join(DATA_FOLDER, "amazonhomo_processed_nvt/part_0.parquet"))
...
save_time_based_splits(data=nvt.Dataset(sessions_gdf),
                       output_dir=os.path.join(DATA_FOLDER, "amazonhomo_preproc_sessions_by_day"),
