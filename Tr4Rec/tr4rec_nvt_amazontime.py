...
DATA_FOLDER=""
#FILENAME_PATTERN ='1.6homosess_final_tsunix1.csv'
#FILENAME_PATTERN ='1.6m_book_homodataset_splitorgts.csv'
FILENAME_PATTERN ='2_feb_20_catg_ts.csv'
...
pandas_df = pd.read_csv(DATA_PATH, usecols=['ItemId', 'Time','SessionId'])
pandas_df.columns = ['item_id', 'timestamp','session_id']  # Rename columns after loading
...
year_index = (
    ...
    nvt.ops.LambdaOp(lambda col: ((col - col.min()) / np.timedelta64(1, 'Y')).astype(int) + 2001) >>
...
workflow.fit_transform(dataset).to_parquet(os.path.join(DATA_FOLDER, "amazontime_processed_nvt"))

workflow.output_schema

workflow.save(os.path.join(DATA_FOLDER, "amazontime_workflow_etl"))

sessions_gdf = cudf.read_parquet(os.path.join(DATA_FOLDER, "amazontime_processed_nvt/part_0.parquet"))

#sessions_gdf = pd.read_parquet(r'/home/pankaj/brijraj/sessions_gdf_mlsecclstr_mod7newday1.parquet')
sessions_gdf = sessions_gdf[sessions_gdf.day_index>=1]

#print(sessions_gdf.head(3))

from transformers4rec.utils.data_utils import save_time_based_splits
save_time_based_splits(data=nvt.Dataset(sessions_gdf),
                       output_dir=os.path.join(DATA_FOLDER, "amazontime_preproc_sessions_by_day"),
