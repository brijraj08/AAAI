...
#loss='bpr-max', final_act='elu-0.5', hidden_act='tanh', layers=[100], adapt='adagrad', n_epochs=10, batch_size=32, dropout_p_embed=0, dropout_p_hidden=0, learning_rate=0.2, momentum=0.1, n_sample=2048, sample_alpha=0, bpreg=0.5, constrained_embedding=True
    ...
    def __init__(self, session_layers, user_layers, n_epochs=10, batch_size=50, learning_rate=0.05, momentum=0.0,
                 ...
                 reset_after_session=True, loss='bpr-max', hidden_act='tanh', final_act='elu-0.5', train_random_order=False,
                 lmbd=0.0, session_key='SessionId', item_key='ItemId', time_key='Time', user_key='UserId', n_sample=0,
                 sample_alpha=0.75, bpreg=0.5,item_embedding=None, init_item_embeddings=None, user_propagation_mode='all',
    ...
    def bpr_max(self, yhat):        ######## Remove_bpr_max########
        softmax_scores = self.softmax_neg(yhat)
        return T.cast(T.sum(-T.log(T.sum(T.nnet.sigmoid(T.diag(yhat)-yhat.T)*softmax_scores, axis=1)+1e-24)+self.bpreg*T.sum((yhat**2)*softmax_scores, axis=1)), theano.config.floatX)
    ...
    def set_item_embeddings(self, E, values):
        ...
        #print(keys)
        #print(list(self.itemidmap.index))
        mask = np.in1d(list(keys), list(self.itemidmap.index), assume_unique=True)
        idx = self.itemidmap.loc[list(keys)].dropna().values.astype(np.int)
        emb = E.get_value()
        emb[idx] = values[mask]
        E.set_value(emb)
