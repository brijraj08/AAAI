...
class RNNRecommender(ISeqRecommender):
    ...
    * Personalizing Session-based Recommendation with Hierarchical Recurrent Neural Networks, Quadrana et al, Recsys 2017
    ...
    #def bert2dict(self, bert_filename="bert_embeddings_plot.txt"):
    def bert2dict(self, bert_filename="bert_embeddings_books.txt"):
        embed_dict = {}
        with open(bert_filename, 'r') as f:
            for line in f:
                line_split = line.strip().split(' ')
                item_id = int(line_split[0])  # Extract item_id directly
                embedding = np.array([float(value) for value in line_split[1:]])  # Convert the rest to a numpy array of floats
                embed_dict[item_id] = embedding
        return embed_dict
    ...
    def fit(self, train_data):
        ...
        if not self.personalized:
            ...
            #embed_dict =self.bert2dict("bert_embeddings_plot.txt")
            embed_dict =self.bert2dict("bert_embeddings_books.txt")
